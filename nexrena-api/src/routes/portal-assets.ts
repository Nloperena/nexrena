...
/** GET /api/portal/assets/:id/url — temporary signed URL for private blob */
router.get('/:id/url', requirePortalAuth, async (req, res) => {
  const contactId = req.portalUser!.contactId
  const asset = await prisma.portalAsset.findFirst({
  ...
  try {
    ...
    console.error('[portal asset url]', err)
