...
export function TeamSidebar({ onOpenSettings }: Props) {
  return (
    <aside className="sticky top-0 z-30 hidden h-screen w-[260px] shrink-0 flex-col border-r border-slate-700/50 bg-[#141820] lg:flex">
      ...
      <nav className="relative flex-1 overflow-y-auto px-2 py-5 min-h-0">
      ...
      <div className="relative border-t border-slate-700/60 px-5 py-4 flex items-center justify-between gap-3">
        {onOpenSettings ? (
          <AccountSettingsButton onClick={onOpenSettings} size="sm" />
        ) : (
          <span />
        )}
        <p className="text-xs text-slate-500 truncate">Nexrena LLC</p>
      </div>
    </aside>
  )
}
