...
export function PortalUploadsSection({ assets, serviceRequests, onUploaded, compact }: Props) {
  ...
  const visibleAssets = compact ? assets.slice(0, 5) : assets

  return (
    <section>
      <h3 className="text-[10px] uppercase tracking-widest text-slate-500 mb-3">Files &amp; Brand Assets</h3>
      <div className="glass-panel rounded-xl border border-slate-800/60 p-5 space-y-4">
        ...
        {assets.length === 0 ? (
          ...
          <ul className="space-y-2">
            ...
            {compact && assets.length > 5 && (
              <li className="text-xs text-slate-500 pt-1">{assets.length - 5} more files on file</li>
            )}
          </ul>
        )}
      </div>
