...
import { PORTAL_LAYOUT_GRID, PORTAL_SIDEBAR_WIDTH, portalFocusRing } from '@/lib/portal-a11y'
...
export function ClientPortalShell({
  ...
  const viewTitle = navItems.find((item) => item.id === activeView)?.label
  ...
  return (
    <div className="client-portal min-h-screen bg-[#111418]">
      <div className={PORTAL_LAYOUT_GRID}>
      <aside
        className={`hidden md:flex sticky top-0 h-screen ${PORTAL_SIDEBAR_WIDTH} flex-col items-center py-4 z-30 border-r border-slate-800/60 bg-[#0c0f12] shrink-0`}
      >
        ...
        <nav className="flex flex-1 flex-col items-center gap-1.5 overflow-y-auto w-full px-2 min-h-0">
        ...
        <div className="mt-2 pt-2 border-t border-slate-800/60 w-full flex justify-center px-2">
      ...
      <div className={`flex flex-col min-h-screen min-w-0 ${isMessenger ? 'min-h-0' : ''}`}>
        ...
        <main
          ...
          className={`flex-1 w-full min-w-0 mx-auto overflow-x-hidden ${
